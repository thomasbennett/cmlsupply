...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex extends Mage_Usa_Model_Shipping_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
	...
	foreach ($items as $item)
	{
		...
		if (!isset($packages[$i]['weight']))
		{
			...
			if ((isset($item['volume'])
			       && ($item['volume'] <= $defaultPackage['volume'])
			       && ($item['weight'] <= ($defaultPackage['weight'] - $defaultPackage['baseline']))) ||
			   (!isset($item['volume'])
			       && ($item['weight'] <= ($defaultPackage['weight'] - $defaultPackage['baseline']))))
			{
		...
		{
		    /** Check if item fits */
		    if ((isset($item['volume']) && isset($packages[$i]['volume'])
			   && (($item['volume'] + $packages[$i]['volume']) <= $packages[$i]['max_volume'])
			   && (($item['weight'] + $packages[$i]['weight']) <= $packages[$i]['max_weight']))
		    || (!isset($item['volume'])
			   && ($item['weight'] + $packages[$i]['weight']) <= $packages[$i]['max_weight']))
		    {
			$packages[$i]['items'][]  = $item;           /** Add item to package */
	...
	elseif (!isset($response->CompletedShipmentDetail->ShipmentRating))
	{
		...
		'package_number'	  => 1,
                ...
                'cod_label_image'         => base64_encode($response->CompletedShipmentDetail->CompletedPackageDetails->CodReturnDetail->Label->Parts->Image),
                'html_image'		  => '');
